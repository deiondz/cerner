...
// ** Import API
import { addHousehold } from "~/api/households/add-household";
...
export function AddPopup({ wards }: AddHouseholdPopupProps) {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      setIsLoading(true);
      const response = await addHousehold(data);
      ...
      if (response.success) {
        ...
        toast.error(response.error ?? "Failed to add household");
      }
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : "Failed to add household",
      );
    } finally {
      setIsLoading(false);
    }
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Enter owner number" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}

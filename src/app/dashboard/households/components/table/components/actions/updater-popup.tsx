...
// ** Import API
import { updateHousehold } from "~/api/households/update-household";
...
export function UpdatePopup({
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      wardId: household.wardId ?? undefined,
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      setIsLoading(true);
      const response = await updateHousehold(data);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Update Household</DialogTitle>
          <DialogDescription>
            Update the household details in the system.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={isLoading}>
                {isLoading ? "Updating..." : "Update household"}
              </Button>
            </DialogFooter>
          </form>
        </Form>

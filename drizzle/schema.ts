...
export const workers = pgTable("workers", {
	workerId: uuid("worker_id").default(sql`uuid_generate_v4()`).primaryKey().notNull(),
	name: text().notNull(),
	contactNumber: text("contact_number").notNull(),
	wardAssigned: text("ward_assigned").notNull(),
	dateCreated: timestamp("date_created", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	dateDeleted: timestamp("date_deleted", { withTimezone: true, mode: 'string' }),
...
export const households = pgTable("households", {
	nfcId: uuid("nfc_id").default(sql`uuid_generate_v4()`).primaryKey().notNull(),
	wardCode: text("ward_code").notNull(),
	householdId: text("household_id").notNull(),
	ownerNumber: text("owner_number").notNull(),
	address: text().notNull(),
	dateCreated: timestamp("date_created", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	dateUpdated: timestamp("date_updated", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	status: text().notNull(),
}, (table) => [
	foreignKey({
...
export const scanlogs = pgTable("scanlogs", {
	scanId: uuid("scan_id").default(sql`uuid_generate_v4()`).primaryKey().notNull(),
...
export const citizenreports = pgTable("citizenreports", {
	reportId: uuid("report_id").default(sql`uuid_generate_v4()`).primaryKey().notNull(),
	nfcId: uuid("nfc_id").notNull(),
